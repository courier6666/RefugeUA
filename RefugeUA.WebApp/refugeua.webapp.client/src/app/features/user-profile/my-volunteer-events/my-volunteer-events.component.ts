...
@Component({
  selector: 'app-my-volunteer-events',
  standalone: false,
  templateUrl: './my-volunteer-events.component.html',
  styleUrl: './my-volunteer-events.component.css'
})
export class MyVolunteerEventsComponent implements OnInit {
  ...
  loadVolunteerEvents() {
    ...
    this.subscription = this.volunteerEventService.getMyVolunteerEvents(this.currentPage, 10, this.prompt).subscribe({
      next: (response) => {
